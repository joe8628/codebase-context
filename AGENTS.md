...
**Writes:** `ARCHITECT_OUTPUT.md`, calls `mem_save` (type: `handoff`).
...
**Writes:** `TASKS.md`, calls `mem_save` (type: `handoff`).
...
**Input:** `ARCHITECT_OUTPUT.md`, `TASKS.md`, `CONVENTIONS.md`, prior `mem_context`.
**Output:** Implementation code in the correct locations.
**Writes:** Source files, calls `mem_save` (type: `handoff`), calls `mem_session_end`.
...
**Writes:** Calls `mem_save` (type: `handoff`).
...
**Output:** Updated docs.
**Writes:** Documentation files, calls `mem_save` (type: `handoff`).
...
**Writes:** Calls `mem_save` (type: `bugfix`).
...
**Writes:** Test files, calls `mem_save` (type: `handoff`).
...
**Writes:** Issue file only — does not call `mem_save`.
